...
class ModelRunner:
    ...
    def profile_max_num_token(self, total_gpu_memory: int):
        ...
        if (
            ...
            # NOTE: need to reserve some memory for RKV operations, currently the value is conservative
            if self.server_args.compress_algorithm is not None:
                # need to multiply a factor for q cache
                cell_size = cell_size * 1.35
                self.mem_fraction_static = self.mem_fraction_static * 0.85
    ...
    def init_memory_pool(
        ...
        if self.server_args.compress_algorithm is not None:
            self.req_to_token_pool.init_q_cache(self.model_config.num_hidden_layers, self.server_args.compress_max_window)
        if (
            self.model_config.attention_arch == AttentionArch.MLA
            and not self.server_args.disable_mla
        ):
