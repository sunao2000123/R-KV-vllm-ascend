...
class NPUModelRunner(GPUModelRunner):
    ...
    def _get_rkv_effective_kv_lens(self, num_reqs: int) -> np.ndarray:
        ...
        for req_idx, req_id in enumerate(self.input_batch.req_ids[:num_reqs]):
            original_len = int(self.input_batch.num_computed_tokens_cpu[req_idx])
            effective_len = self.rkv_effective_kv_lens.get(req_id)
    ...
    def _iter_rkv_attn_metadata(self, attn_metadata: PerLayerAttnMetadata):
        if isinstance(attn_metadata, list):
            for ubatch_metadata in attn_metadata:
                yield from ubatch_metadata.values()
        else:
    ...
    def _update_rkv_effective_kv_lens(
        ...
        if not self.rkv_current_enabled:
            return
    ...
    def _prepare_inputs(
        ...
        slot_positions_np = positions_np
        if self.rkv_current_enabled:
            slot_positions_np = np.empty_like(positions_np)
            np.add(self.rkv_effective_kv_lens_before[req_indices], arange, out=slot_positions_np)
        ...
        if self.rkv_current_enabled:
            self.rkv_seq_lens.np[:num_reqs] = self.rkv_effective_kv_lens_before + num_scheduled_tokens
            self.rkv_seq_lens.cpu[num_reqs:].fill_(0)
            self.rkv_seq_lens.copy_to_gpu()
    ...
    def execute_model(
        ...
        self._update_rkv_effective_kv_lens(attn_metadata, num_scheduled_tokens_np)
    ...
    def _build_attention_metadata(
        ...
        seq_lens_buffer = self.rkv_seq_lens if self.rkv_current_enabled else self.seq_lens
        if for_cudagraph_capture:
            ...
            max_seq_len = seq_lens_buffer.np[:num_reqs].max().item()
        ...
        cm_base = AscendCommonAttentionMetadata(
            ...
            seq_lens=seq_lens_buffer.gpu[:num_reqs_padded],
            # TODO
            seq_lens_cpu=seq_lens_buffer.cpu[:num_reqs_padded],
            # TODO
